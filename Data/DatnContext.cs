...
#warning To protect potentially sensitive information in your connection string, you should move it out of source code. You can avoid scaffolding the connection string by using the Name= syntax to read it from configuration - see https://go.microsoft.com/fwlink/?linkid=2131148. For more guidance on storing connection strings, see https://go.microsoft.com/fwlink/?LinkId=723263.
        => optionsBuilder.UseSqlServer("Data Source=LAPTOP-FDN5AM5O\\SQLEXPRESS;Initial Catalog=DATN1;Integrated Security=True;Pooling=False;Encrypt=True;Trust Server Certificate=True;MultipleActiveResultSets=true");

    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        // Cấu hình mối quan hệ giữa Promotion và ShippingProvider
        modelBuilder.Entity<ShippingProvider>()
            .HasOne(sp => sp.Promotion)  // Mỗi ShippingProvider thuộc về một Promotion
            .WithMany(p => p.ShippingProviders)  // Mỗi Promotion có thể có nhiều ShippingProviders
            .HasForeignKey(sp => sp.PromoCode)  // Khóa ngoại tham chiếu đến PromoCode của Promotion
            .OnDelete(DeleteBehavior.Restrict);  // Tùy chọn xử lý khi Promotion bị xóa (có thể là Restrict, Cascade, hoặc SetNull)
